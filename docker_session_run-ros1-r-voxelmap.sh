...
IMAGE_NAME='r-voxelmap_noetic'
TMUX_SESSION='ros1_session'

DATASET_CONTAINER_PATH='/ros_ws/dataset/recorded-r-voxelmap.bag'
BAG_OUTPUT_CONTAINER='/ros_ws/recordings'

RECORDED_BAG_NAME="recorded-r-voxelmap.bag"
...
echo "=== R-VoxelMap rosbag pipeline ==="
...
roslaunch rvoxelmap avia.launch use_sim_time:=true
'\'' C-m
...
source /ros_ws/devel/setup.bash
echo "[record] start"
rosbag record /rvoxelmap/cloud_registered /rvoxelmap/aft_mapped_to_init -O '"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME"'
echo "[record] exit"
'\'' C-m
...
docker run -it --rm \
  ...
  /bin/bash -c "
    ...
    source /opt/ros/noetic/setup.bash
    source /ros_ws/devel/setup.bash
    rosrun r-voxelmap-to-hdmapping listener \
      \"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME\" \
      \"$BAG_OUTPUT_CONTAINER/$HDMAPPING_OUT_NAME-r-voxelmap\"
  "
